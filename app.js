...
const filterFields = {
  streams: ["年", "チャンネル", "タイプ", "ソロ/コラボ", "ゲーム", "コラボメンバー", "再生リスト"],
  games: ["タイプ"],
  collaborators: ["グループ", "所属"],
...
function defaultProps(extra = []) {
  return [
    { icon: "👤", label: "Owner", value: "リー ペイイー" },
    { icon: "✅", label: "Verification", value: "" },
    { icon: "🏷️", label: "Tags", value: "" },
...
function renderTable(slug, initialFilter = "", mode = "", month = "", fieldFilters = {}) {
  ...
  app.innerHTML = `
    ...
    <div class="table-shell">
      <table>
        <thead>
          <tr>${payload.fields.map((field) => headerCell(field)).join("")}</tr>
        </thead>
...
function headerCell(field) {
  ...
  return `<th><button class="sort-button" type="button" data-field="${escapeAttribute(field)}">${fieldIcon(field)} ${escapeHtml(field)}${arrow}</button></th>`;
}
    ...
    <tr>
      ${fields.map((field) => {
        ...
        if (field === titleField) {
          ...
          content = cellContent(slug, field, value);
        }
...
function relationChips(value, targetSlug) {
  const parts = splitRelation(value);
  if (!parts.length) return "";
  return `<span class="chips">${parts.map((name) => relationChip(name, targetSlug)).join("")}</span>`;
}
